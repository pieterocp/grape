  ...
  describe "coerce" do
    ...
    context "coerces" do
      ...
      context "Array" do
        ...
        it "Array of Bools" do
          subject.params do
            requires :arry, coerce: [Grape::API::Boolean]
          end
          subject.get "/array" do
            params[:arry][0].class
          end
        ...
        it "Array of a custom type" do
          context = self
          subject.params do
            requires :uri, type: [context.secure_uri_only]
          end
    ...
    context "using coerce_with" do
      ...
      it "parses parameters with Array[String] type" do
        subject.params do
          requires :values, type: [String], coerce_with: ->(val) { val.split(/\s+/) }
        end
      ...
      it "parses parameters with Array[Array[String]] type and coerce_with" do
        subject.params do
          requires :values, type: [[String]], coerce_with: ->(val) { val.is_a?(String) ? [val.split(",").map(&:strip)] : val }
        end
      ...
      it "parses parameters with Array[Integer] type" do
        subject.params do
          requires :values, type: [Integer], coerce_with: ->(val) { val.split(/\s+/).map(&:to_i) }
        end
        subject.get "/ints" do
          params[:values]
        end
